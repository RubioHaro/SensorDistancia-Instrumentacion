...
function App() {
  ...
  const [value1, setValue1] = React.useState(23);
  const [value2, setValue2] = React.useState(24);
  ...
  const updateDistance = (event) => {
    ...
    setValue(parsed_string);

    if (selected == 1) { // kg
      setValue1(value) //: calc(value) calculos para peso
    } else if (selected == 2) { // °C
      setValue2(value)//: calc(value) calculos para temperatura
    } else if (selected == 3) { // l
      setValue3(value)//: calc(value) calculos para volumen
    }
  ...
  const handleSelect = (event) => {
    ...
    if (toSend == 0) {
      toSend = 1;
      fetch('http://localhost:5000/selected0', {
        method: 'POST',
        body: {}
      })
    } else if (toSend == 1) {
      fetch('http://localhost:5000/selected1', {
        method: 'POST',
        body: {}
      })
    } else if (toSend == 2) {
